...
$SQLFirewallOperation = "Microsoft.Sql/servers/firewallRules/write"
...
if (@($PassingAlerts).Count -gt 0) {
    Write-Host "** PASS : Activity Log Alert exists for Create or Update SQL Server Firewall Rule. **"
    $PassingAlerts | Format-List
} else {
    Write-Host "** FAIL : No enabled Activity Log Alert found for Create or Update SQL Server Firewall Rule with an action group. **"
