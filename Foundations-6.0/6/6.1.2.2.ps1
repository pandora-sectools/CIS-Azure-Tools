...
foreach ($Alert in $Alerts) {
    ...
    if ($Obj.OperationName.equals -notmatch $DeletePolicyAssignment ) { $Obj.AuditState = "FAIL"}
...
if (@($PassingAlerts).Count -gt 0) {
    Write-Host "** PASS : Activity Log Alert exists for Delete Policy Assignment. **"
    $PassingAlerts | Format-List
} else {
    Write-Host "** FAIL : No enabled Activity Log Alert found for Delete Policy Assignment with an action group. **"
