# 6.1.2.5 Ensure that Activity Log Alert Exists for Delete Security Solution (Automated)
# Level 1
...
$SecuritySolutionOperation = "Microsoft.Security/securitySolutions/delete"
...
foreach ($Alert in $Alerts) {
    ...
    if ($Obj.OperationName.equals -notmatch $SecuritySolutionOperation ) { $Obj.AuditState = "FAIL"}
...
if (@($PassingAlerts).Count -gt 0) {
    Write-Host "** PASS : Activity Log Alert exists for Deleting Security Solution. **"
    $PassingAlerts | Format-List
} else {
    Write-Host "** FAIL : No enabled Activity Log Alert found for Deleting Security Solution with an action group. **"
