...
$NSGDeletePolicyAssignment = "Microsoft.Network/networkSecurityGroups/delete"
...
foreach ($Alert in $Alerts) {
    ...
    if ($Obj.OperationName.equals -notmatch $NSGDeletePolicyAssignment ) { $Obj.AuditState = "FAIL"}
...
if (@($PassingAlerts).Count -gt 0) {
    Write-Host "** PASS : Activity Log Alert exists for Delete Network Security Group. **"
    $PassingAlerts | Format-List
} else {
    Write-Host "** FAIL : No enabled Activity Log Alert found for Delete Network Security Group with an action group. **"
