...
$ServiceHealthCategory = "ServiceHealth"
...
foreach ($Alert in $Alerts) {
    ...
    $Obj = [PSCustomObject]@{    
        ...
        Category                   = $Alert.Condition | Where-Object { $_.field -eq "category" }
        ActionGroupAssigned        = @($Alert.Actions.actionGroups).Count -gt 0
        AuditState                 = "PASS"
    }

    if ($Obj.Enabled -ne $true) { $Obj.AuditState = "FAIL" }
    if ($Obj.Category.equals -notmatch $ServiceHealthCategory) { $Obj.AuditState = "FAIL" }
...
if (@($PassingAlerts).Count -gt 0) {
    Write-Host "** PASS : Activity Log Alert exists for Service Health. **"
    $PassingAlerts | Format-List
} else {
    Write-Host "** FAIL : No enabled Activity Log Alert found for Service Health with an action group. **"
