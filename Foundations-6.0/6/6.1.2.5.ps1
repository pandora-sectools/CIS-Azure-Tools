...
$SecuritySolutionOperation = "Microsoft.Security/securitySolutions/write"
...
if (@($PassingAlerts).Count -gt 0) {
    Write-Host "** PASS : Activity Log Alert exists for Create or Update Security Solution. **"
    $PassingAlerts | Format-List
} else {
    Write-Host "** FAIL : No enabled Activity Log Alert found for Create or Update Security Solution with an action group. **"
