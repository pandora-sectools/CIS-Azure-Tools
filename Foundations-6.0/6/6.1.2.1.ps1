...
foreach ($Alert in $Alerts) {
    ...
    if ($Obj.OperationName.equals -notmatch $CreatePolicyAssignment) {$Obj.AuditState = "FAIL"}
...
if (@($PassingAlerts).Count -gt 0) {
    Write-Host "** PASS : Activity Log Alert exists for Create Policy Assignment. **"
    $PassingAlerts | Format-List
} else {
    Write-Host "** FAIL : No enabled Activity Log Alert found for Create Policy Assignment with an action group. **"
