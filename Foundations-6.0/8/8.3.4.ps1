...
if (@($FailingSecrets).Count -gt 0) {
    Write-Host "`n** FAIL : Enabled Key Vault secrets found without an expiration date in Key Vaults using Access Policies (Legacy). **"
...
elseif (@($AccessIssues).Count -gt 0) {
    ...
    Write-Host "`n** PASS : All enabled Key Vault secrets in Key Vaults using Access Policies (Legacy) have an expiration date set. **"
